...
describe('permission smoke checks', () => {
  it('auditor persona can access report-view program routes but not manager-only pages', () => {
    ...
    expect(canAccessRoute('/admin/findings', auditor)).toBe(true)
    expect(canAccessRoute('/admin/issue-program', auditor)).toBe(true)
    expect(canAccessRoute('/admin/risk-register', auditor)).toBe(true)
